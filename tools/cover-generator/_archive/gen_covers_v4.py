#!/usr/bin/env python3
"""
Cover Generator v4 — Multi-Format Marketing Asset Generator

Formats:
  cover        800x800   Gumroad / Stripe product cover
  ig-post      1080x1080 Instagram Feed / Square
  fb-post      1200x630  Facebook Post / OG Image / LinkedIn Post
...
Usage:
  python3 gen_covers_v4.py                         # all products, cover format
  python3 gen_covers_v4.py localai-playbook         # single product
  python3 gen_covers_v4.py --format fb-post         # single format
  python3 gen_covers_v4.py --format cover,ig-post   # multiple formats
  python3 gen_covers_v4.py --format all             # all formats
  python3 gen_covers_v4.py --list                   # list products & formats
"""
...
def eagle_div(size_px, top_pct, right_val=None, extra_css=""):
    if not EAGLE_B64:
        return ""
    pos = f"top:{top_pct}%;left:50%;transform:translate(-50%,-50%)"
    if right_val is not None:
        pos = f"top:50%;right:{right_val};transform:translateY(-50%)"
    return (f'<div style="position:absolute;width:{size_px}px;height:{size_px}px;'
            f'{pos};opacity:0.11;z-index:4;{extra_css}">'
            f'<img src="{EAGLE_B64}" style="width:100%;height:100%;object-fit:contain" alt=""></div>')


# --- SQUARE (cover 800x800, ig-post 1080x1080) ---
...
.orb1{{position:absolute;width:{int(600*s)}px;height:{int(600*s)}px;background:radial-gradient(circle,var(--a22) 0%,transparent 65%);top:{int(-150*s)}px;left:{int(-150*s)}px;z-index:2}}
.orb2{{position:absolute;width:{int(450*s)}px;height:{int(450*s)}px;background:radial-gradient(circle,var(--a11) 0%,transparent 65%);bottom:{int(-100*s)}px;right:{int(-100*s)}px;z-index:2}}
...
.lline{{position:absolute;left:0;top:0;bottom:0;width:3px;background:linear-gradient(to bottom,transparent,var(--accent),transparent);z-index:5}}
...
.badge{{display:inline-flex;align-items:center;gap:7px;background:var(--a20);border:1px solid var(--a50);border-radius:100px;padding:6px 16px;color:var(--accent);font-size:{round(11.5*s,1)}px;font-weight:600;letter-spacing:.07em;text-transform:uppercase}}
.bdot{{width:5px;height:5px;background:var(--accent);border-radius:50%}}
.ibox{{width:{int(44*s)}px;height:{int(44*s)}px;background:var(--a18);border:1px solid var(--a35);border-radius:{int(10*s)}px;display:flex;align-items:center;justify-content:center;font-size:{int(20*s)}px}}
...
.title{{font-family:'Space Grotesk',sans-serif;font-size:{int(66*s)}px;font-weight:700;line-height:1.04;color:#F8FAFC;letter-spacing:-.02em;margin-bottom:{int(22*s)}px}}
.title em{{color:var(--accent);font-style:normal}}
.sub{{font-size:{round(16.5*s,1)}px;color:rgba(248,250,252,.55);line-height:1.65;max-width:{int(470*s)}px;margin-bottom:{int(36*s)}px}}
...
.sp{{flex:1}}
.div{{height:1px;background:linear-gradient(to right,var(--a40),transparent);margin-bottom:{int(24*s)}px}}
.bot{{display:flex;align-items:center;justify-content:space-between}}
.stats{{display:flex;gap:{int(36*s)}px}}
.snum{{font-size:{int(24*s)}px;font-weight:700;color:var(--accent);line-height:1}}
.slbl{{font-size:{round(10.5*s,1)}px;color:rgba(255,255,255,.35);margin-top:4px;text-transform:uppercase;letter-spacing:.06em}}
.price{{background:linear-gradient(135deg,var(--accent),var(--accent-dark));border-radius:{int(12*s)}px;padding:{int(13*s)}px {int(30*s)}px;font-size:{int(28*s)}px;font-weight:800;color:#fff}}
...
# --- LANDSCAPE (fb-post 1200x630, twitter-post 1200x675, yt-thumb 1280x720) ---
def build_landscape(p, fmt):
    w, h = fmt["w"], fmt["h"]
    sw, sh = w/1200, h/630
    feats = "".join(f'<div class="feat">{f}</div>' for f in p["features"])
    stats = "".join(f'<div class="stat"><div class="snum">{n}</div><div class="slbl">{l}</div></div>' for n,l in p["stats"])
    eg = eagle_div(int(460*sw), 50, right_val=f"{int(60*sw)}px")
    return f"""<!DOCTYPE html><html><head><meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Space+Grotesk:wght@600;700&display=swap" rel="stylesheet">
<style>
*{{margin:0;padding:0;box-sizing:border-box}}:root{{{css_vars(p)}}}
...
.vdiv{{position:absolute;top:8%;bottom:8%;left:60%;width:1px;background:linear-gradient(to bottom,transparent,var(--a40),transparent);z-index:4}}
.lline{{position:absolute;left:0;top:0;bottom:0;width:3px;background:linear-gradient(to bottom,transparent,var(--accent),transparent);z-index:5}}
...
.badge{{display:inline-flex;align-items:center;gap:6px;background:var(--a20);border:1px solid var(--a50);border-radius:100px;padding:5px 14px;color:var(--accent);font-size:{round(10.5*sw,1)}px;font-weight:600;letter-spacing:.07em;text-transform:uppercase;margin-bottom:{int(16*sh)}px;width:fit-content}}
...
.title{{font-family:'Space Grotesk',sans-serif;font-size:{int(52*sw)}px;font-weight:700;line-height:1.08;color:#F8FAFC;letter-spacing:-.02em;margin-bottom:{int(14*sh)}px}}
.title em{{color:var(--accent);font-style:normal}}
.sub{{font-size:{round(13.5*sw,1)}px;color:rgba(248,250,252,.5);line-height:1.6;max-width:{int(460*sw)}px;margin-bottom:{int(18*sh)}px}}
...
.snum{{font-size:{int(26*sw)}px;font-weight:700;color:var(--accent);line-height:1}}
.slbl{{font-size:{round(9*sw,1)}px;color:rgba(255,255,255,.3);margin-top:3px;text-transform:uppercase;letter-spacing:.06em}}
.price{{background:linear-gradient(135deg,var(--accent),var(--accent-dark));border-radius:{int(12*sw)}px;padding:{int(14*sh)}px {int(28*sw)}px;font-size:{int(30*sw)}px;font-weight:800;color:#fff;text-align:center;width:100%}}
.brand{{font-size:{int(9*sw)}px;color:rgba(255,255,255,.2);letter-spacing:.1em;text-transform:uppercase}}
</style></head><body><div class="wrap">
<div class="orb1"></div><div class="gbg"></div>
{eg}
...
<div class="right">
  ...
  <div class="brand">ai-engineering.at</div>
</div>
</div></body></html>"""
...
.orb1{{position:absolute;width:900px;height:900px;background:radial-gradient(circle,var(--a22) 0%,transparent 65%);top:-300px;left:50%;transform:translateX(-50%);z-index:1}}
.orb2{{position:absolute;width:700px;height:700px;background:radial-gradient(circle,var(--a15) 0%,transparent 65%);bottom:-200px;left:50%;transform:translateX(-50%);z-index:1}}
...
.lline-l{{position:absolute;left:0;top:0;bottom:0;width:3px;background:linear-gradient(to bottom,transparent,var(--accent),transparent);z-index:5}}
.lline-r{{position:absolute;right:0;top:0;bottom:0;width:3px;background:linear-gradient(to bottom,transparent,var(--accent),transparent);z-index:5}}
...
.badge{{display:inline-flex;align-items:center;gap:8px;background:var(--a20);border:1px solid var(--a50);border-radius:100px;padding:9px 24px;color:var(--accent);font-size:14px;font-weight:600;letter-spacing:.08em;text-transform:uppercase}}
...
.title{{font-family:'Space Grotesk',sans-serif;font-size:88px;font-weight:700;line-height:1.05;color:#F8FAFC;letter-spacing:-.02em;margin-bottom:30px}}
.title em{{color:var(--accent);font-style:normal}}
.sub{{font-size:20px;color:rgba(248,250,252,.48);line-height:1.6;max-width:840px;margin:0 auto 42px}}
...
.bot{{position:relative;z-index:10;display:flex;flex-direction:column;align-items:center;gap:28px;width:100%}}
...
.snum{{font-size:30px;font-weight:700;color:var(--accent);line-height:1}}
.slbl{{font-size:11px;color:rgba(255,255,255,.28);margin-top:5px;text-transform:uppercase;letter-spacing:.07em}}
.price{{background:linear-gradient(135deg,var(--accent),var(--accent-dark));border-radius:16px;padding:20px 70px;font-size:42px;font-weight:800;color:#fff}}
...
<div class="top">
  <div class="brand-top">ai-engineering.at</div>
  <div class="badge"><span class="bdot"></span>{p['badge']}</div>
</div>
<div class="mid">
  <div class="ey">{p['eyebrow']}</div>
...
<div class="bot">
  <div class="stats">{stats}</div>
  <div class="price">{p['price']}</div>
  <div class="brand-bot">ai-engineering.at</div>
</div>
</div></body></html>"""


# --- BANNER (fb-banner 820x312, li-banner 1584x396) ---
def build_banner(p, fmt):
    w, h = fmt["w"], fmt["h"]
    sw, sh = w/1584, h/396
    stats = "".join(f'<div class="stat"><div class="snum">{n}</div><div class="slbl">{l}</div></div>' for n,l in p["stats"][:2])
    eg = eagle_div(int(340*sw), 50, right_val=f"{int(290*sw)}px")
...
.brand-name{{font-family:'Space Grotesk',sans-serif;font-size:{int(15*sw)}px;font-weight:700;color:var(--accent)}}
...
.badge{{display:inline-flex;align-items:center;gap:5px;background:var(--a20);border:1px solid var(--a50);border-radius:100px;padding:3px 10px;color:var(--accent);font-size:{round(8*sw,1)}px;font-weight:600;letter-spacing:.07em;text-transform:uppercase;margin-bottom:{int(7*sh)}px;width:fit-content}}
.bdot{{width:3px;height:3px;background:var(--accent);border-radius:50%}}
.title{{font-family:'Space Grotesk',sans-serif;font-size:{int(32*sw)}px;font-weight:700;line-height:1.1;color:#F8FAFC;letter-spacing:-.01em;margin-bottom:{int(5*sh)}px}}
.title em{{color:var(--accent);font-style:normal}}
...
.snum{{font-size:{int(17*sw)}px;font-weight:700;color:var(--accent);line-height:1}}
.slbl{{font-size:{round(7*sw,1)}px;color:rgba(255,255,255,.28);margin-top:2px;text-transform:uppercase;letter-spacing:.06em}}
.price{{background:linear-gradient(135deg,var(--accent),var(--accent-dark));border-radius:{int(8*sw)}px;padding:{int(8*sh)}px {int(20*sw)}px;font-size:{int(20*sw)}px;font-weight:800;color:#fff;white-space:nowrap}}
</style></head><body><div class="wrap">
<div class="orb1"></div><div class="gbg"></div>
{eg}
<div class="lline"></div>
...
def render(p, fmt_name, fmt, out_dir):
    ...
    w, h = fmt["w"], fmt["h"]
    hp = f"/tmp/cv4-{p['id']}-{fmt_name}.html"
    op = out_dir / f"{p['id']}-{fmt_name}.png"
...
def main():
    parser = argparse.ArgumentParser(description="Cover Generator v4 — Multi-Format",
    ...
    print(f"\nGeneriere {len(products)} Produkt(e) x {len(fmt_names)} Format(e) = {total} Assets\n")
